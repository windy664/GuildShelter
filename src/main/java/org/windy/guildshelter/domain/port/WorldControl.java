 ...
 * 于是平整地皮生不出来。因此世界创建/生成将改由 <b>NeoForge 侧动态维度 + 自定义 ChunkGenerator</b>
...
public interface WorldControl {
    ...
    /** 公会世界名（如 {@code guild_<id>}），平台无关，两侧需一致。 */
    String worldName(GuildId guild);

    /**
     * 创建（或加载已存在的）公会世界，并同步出生点与边界。须在主线程调用。
     *
